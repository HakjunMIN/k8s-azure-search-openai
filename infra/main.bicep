...
param aksName string = '' // Set in main.parameters.json
...
resource acrResourceGroup 'Microsoft.Resources/resourceGroups@2021-04-01' existing = if (!empty(acrResourceGroupName)) {
  name: !empty(acrResourceGroupName) ? acrResourceGroupName : resourceGroup.name
}
...
resource keyvaultResourceGroup 'Microsoft.Resources/resourceGroups@2021-04-01' existing = if (!empty(searchServiceResourceGroupName)) {
  name: !empty(searchServiceResourceGroupName) ? searchServiceResourceGroupName : resourceGroup.name
}
...
module keyvault 'core/security/keyvault.bicep' = {
  ...
  scope: resourceGroup
  params: {
    name: !empty(openAiServiceName) ? openAiServiceName : '${abbrs.keyVaultVaults}${resourceToken}'
    location: location
    tags: tags
...
module aks 'core/host/aks.bicep' = {
  ...
  scope: resourceGroup
  params: {
    name: !empty(aksName) ? aksName : '${abbrs.kubernetesConnectedClusters}${resourceToken}'
    containerRegistryName: acr.outputs.name
    location: location
    tags: tags  
    nodeResourceGroupName: '${abbrs.resourcesResourceGroups}${resourceToken}'
...
// module kvRoleUser 'core/security/role.bicep' = {
//   scope: searchServiceResourceGroup
...
// module acrpull 'core/security/registry-access.bicep' = {
//   scope: acrResourceGroup
//   name: 'acr-pull-role'
//   params: {
//     containerRegistryName: acr.outputs.name
//     principalId: aks.outputs.clusterIdentity.clientId
//   }
// }
...
var otherPrivateEndpointConnections = usePrivateEndpoint ? [
  ...
  }
  {
    ...
    resourceIds: [ aks.outputs.clusterResourceId ]
  }
] : []
